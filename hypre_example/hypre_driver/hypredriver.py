...
def exucute(params, RUNDIR, niter = 1):
    # extract arguments
    ...
    myargslist = [Problem, '-n', f'{nx}', f'{ny}', f'{nz}', coeffs_c, coeffs_a, '-solver', f'{solver}', '-P', f'{Px}', f'{Py}', f'{Pz}', '-th', f'{strong_threshold}', '-tr', f'{trunc_factor}', 
                  '-Pmx', f'{P_max_elmts}', '-rlx', f'{relax_type}', '-smtype', f'{smooth_type}', '-smlv', f'{smooth_num_levels}', '-interptype', f'{interp_type}', '-agg_nl', f'{agg_num_levels}', CoarsType, "> ", outputfilename]
    ...
    def v_sequential():
        ...
        Procs = "export OMP_PLACES=threads; export OMP_PROC_BIND=spread; export OMP_NUM_THREADS=1; srun --ntasks-per-node %d " % NProc
        command = Procs + EXCUDIR + myargs + "> " + outputfilename
        runtimes = []
        for i in range(niter):
            print("Sequentially execute the following ......\n")
            print(command)
            os.system(command)
            runtime = read_output(outputfilename)
            print(f"Interation {i}, runtime = {runtime}")
            runtimes.append(runtime)
        return np.mean(runtimes) 
    
    '''
    TODOS:
        1. write output(runtime) into outputfilename
        2. run niter times and take average time 
    '''
    def v_parallel():
        nthreads = 1
    ...
    runtime = v_sequential()
    return runtime

def hypredriver(params, niter = 10, JOBID: int=-1):
    global EXPDIR 
    global ROOTDIR
    ...
    for param in params:
        ...
        time_cur = exucute(param, RUNDIR, niter=niter)
